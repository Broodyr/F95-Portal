...
- All state services are singletons with injectable storage/clients for tests; 209 tests, `flutter analyze` clean.
...
- Forum stack: `lib/screens/forum_screen.dart` (directory tab: grouped category sections, link nodes hidden) → `lib/screens/forum_threads_screen.dart` (subforum block reusing `lib/widgets/forum_node_row.dart` + splitter + infinite-scroll thread rows, stickies deduped on later pages) → `lib/screens/forum_thread_screen.dart` (full-screen viewer: post cards w/ `ForumAvatar` initials/images, quote blocks, collapsible spoilers via SlidingReveal, reaction chip → `lib/widgets/reactions_sheet.dart` w/ per-reaction filter pills; pagination pills). Reaction ids map to glyphs in `lib/widgets/reaction_icon.dart` (Material Symbols via `material_symbols_icons` — the current Google design, unlike Flutter's frozen classic `Icons.*`; a few reactions render as system emoji `Text` since those codepoints aren't in any icon font and would break release tree-shaking). Read-only for now (react/reply actions planned; react = existing `postAction` w/ `reaction_id=N`). All fetches injectable; screens run on service mocks in tests/web. glass strip over the list when filtering: count, removable per-filter chips, clear-all (resets to settings defaults).
...
## Development
- `flutter pub get && flutter run`; `flutter test` (209 tests); `flutter analyze`. Helper scripts in `tool/`.
