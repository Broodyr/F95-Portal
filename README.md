...
Unofficial Android client for F95zone. Browse and search the latest-updates feed with full filtering functionality; read, post, and react in the forums; keep up with bookmarks and alerts. All in a native dark, glassmorphic UI built with Flutter.

> **18+ only.** F95zone hosts adult content, so this app is for adults. It is not affiliated with or endorsed by F95zone.

## Install

Download the latest `app-release.apk` from [Releases](../../releases) and side-load it. Android will ask you to allow installs from whichever app you opened it with (your browser or file manager) the first time. Requires Android 7.0 or newer.

There's no Play Store listing — Google doesn't like adult content :( — and no iOS build, since I don't have a Mac to build or test on. If you have one and want to help get iOS working, [open an issue](../../issues).
...
| `lib/utils/` | Small pure helpers: text/number formatters and image-URL rewriting. |
| `lib/theme/`, `lib/constants.dart` | Colors and shared design tokens: blur tiers, radii, motion, mock latency. |
...
- Needs Flutter with Dart SDK 3.8+. `flutter pub get && flutter run`; `flutter test`; `flutter analyze`. Helper scripts in `tool/`.
