...
- Thread details are scraped live: `lib/services/thread_page_parser.dart` parses the first post (tolerant block model — meta fields, overview, rich spoiler sections with formatting/links/inline images, download sets incl. alternates/torrents/attachments, extras) and `lib/services/thread_page_service.dart` fetches/caches it; the modal renders whatever blocks exist plus like/bookmark buttons (XenForo react/watch endpoints, logged-in only). Parser is TDD'd against `test/fixtures/*.htm`.
